...
public class XML {
                ...
                if (token instanceof String) {
                    ...
                    if (config.getForceList().contains(tagName)) {
                        ...
                        if(!tagName.equals(targetTag)) {
                            ...
                            if (nilAttributeFound) {
                                res.accumulate(tagName, JSONObject.NULL);
                            } else if (jsonObject.length() > 0) {
     ...
     * @throws JSONException Thrown if any parsing error occurs.
     */
    private static boolean parse4(XMLTokener x, JSONObject context, String name, XMLParserConfiguration config, int currentNestingDepth, Function<String, String> function)
            throws JSONException {
        char c;
        int i;
        JSONObject jsonObject = null;
        ...
        if (token == BANG) {
            ...
            tagName = (String) token;
            String newTagName = function.apply(tagName);
            token = null;
            jsonObject = new JSONObject();
            boolean nilAttributeFound = false;
            xmlXsiTypeConverter = null;
                ...
                if (token instanceof String) {
                    ...
                    for (;;) {
                        ...
                        if (token == null) {
                            ...
                            if (parse4(x, jsonObject, tagName, config, currentNestingDepth + 1, function)) {
                                if (config.getForceList().contains(tagName)) {
                                    // Force the value to be an array
                                    if (jsonObject.length() == 0) {
                                        context.put(newTagName, new JSONArray());
    ...
    public static JSONObject toJSONObject(Reader reader, Function<String, String> function) {
        ...
        while (x.more()) {
            x.skipPast("<");
            if(x.more()) {
                parse4(x, jo, null, XMLParserConfiguration.ORIGINAL, 0, function);
            }
        }
        return jo;
    }
